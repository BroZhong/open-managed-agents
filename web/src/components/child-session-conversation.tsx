...
export function ChildSessionConversation({ sessionId, workspaceId, onOpenExecution, onOpenWorkspaceFile }: { sessionId: string; workspaceId?: string; onOpenExecution?: (execution: DelegationExecution) => void; onOpenWorkspaceFile?: (path: string) => void }) {
  ...
  const { events, activeDeltas, status, isHistoryLoading, historyError } = useSessionEvents(sessionId);
  const effectiveStatus = session?.status === "terminated" ? "idle" : status;
  if (isError) return <p role="alert" className="px-6 py-2 text-xs">Could not load child Session. <button className="underline" onClick={() => void refetch()}>Retry</button></p>;
  return <div className="flex h-full min-h-0 flex-col">
    ...
    <div className="min-h-0 flex-1">
      <ConversationView loading={isLoading || isHistoryLoading} loadError={historyError} onOpenExecution={onOpenExecution} resources={session ? { agentId: session.agentId, skills, onOpenWorkspacePath: session.workspaceId === workspaceId ? onOpenWorkspaceFile : undefined } : undefined} sessionId={sessionId} events={events} activeDeltas={activeDeltas} sessionStatus={effectiveStatus} />
    </div>
