...
describe("EquipPicker optimistic updates", () => {
  it("waits for the Agent's equipped Skills before enabling toggles", async () => {
    ...
    const toggle = () => screen.getByRole("checkbox", { name: /Loading Skill/ });
  ...
  it("offers a retry when the Agent's equipped Skills fail to load", async () => {
    ...
    const toggle = () => screen.getByRole("checkbox", { name: /Retry Skill/ });
  ...
  it("keeps toggles disabled across a remount while a Skill write is pending", async () => {
    ...
    fireEvent.click(screen.getByRole("checkbox", { name: /Remount Skill/ }));
    ...
    const remountedToggle = () => screen.getByRole("checkbox", { name: /Remount Skill/ });
    try {
  ...
  it("shows a Skill as equipped before its fork request finishes", async () => {
    ...
    queryClient.setQueryData(["skills"], [
      librarySkill,
      { ...librarySkill, id: "skill_other", name: "Other Skill" },
    ]);
    queryClient.setQueryData(["agents", agent.id, "skills"], []);
    ...
    const toggle = () => screen.getByRole("checkbox", { name: /Fast Skill/ });
    ...
    try {
      ...
      const otherToggle = screen.getByRole("checkbox", { name: "Enable Other Skill" });
      expect((otherToggle as HTMLInputElement).checked).toBe(false);
      expect(otherToggle.hasAttribute("disabled")).toBe(true);
      expect(screen.getByRole("button", { name: "Update Fast Skill" }).hasAttribute("disabled")).toBe(true);
      // Native clicks on disabled controls cannot enqueue duplicate writes or
      // accidentally equip another Library Skill while the fork is pending.
      toggle().click();
      otherToggle.click();
      expect(fetchMock).toHaveBeenCalledTimes(1);
      expect(
  ...
  it("rolls back the optimistic check when the fork request fails", async () => {
    ...
    const toggle = () => screen.getByRole("checkbox", { name: /Rollback Skill/ });
    fireEvent.click(toggle());
    await waitFor(() =>
      expect((toggle() as HTMLInputElement).checked).toBe(true),
    );
