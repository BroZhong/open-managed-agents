...
**Runtime CLI Credential**:
An optional credential and allowlisted project context supplied with an authenticated user event and stored out-of-band under that event's generated Pending Event id. The supported headers are `X-VFS-Token`, `X-VFS-Project-Url`, `X-VFS-Project-Id`, `X-VFS-Teamwork-Id`, `X-VFS-Storyboard-Id`, and `X-VFS-Runtime-Env`. The Host validates the context, resolves it after the exact pending claim, exposes it as `VFS_TOKEN`, `VFS_PROJECT_URL`, `VFS_PROJECT_ID`, `VFS_TEAMWORK_ID`, `VFS_STORYBOARD_ID`, and `RUNTIME_ENV` only to one direct `vfs-cli` subprocess, and deletes it after acknowledgement. The token is never model-visible; the same non-secret project context is also supplied in one transient system-prompt block for that Turn so the Agent can use identifiers that the CLI does not resolve from environment. An Agent with `sandbox.credentialMode: runtime-vfs` also strips `VFS_TOKEN` from legacy deployment/Agent create-time env, preventing fallback to a sandbox-global identity. These values never enter the Pending Event, Event Log, Agent, Workspace, sandbox creation environment, or arbitrary shell environment. The client supplies the current values on every user message so token/project rotation and multi-Host claim handoff remain correct.
_Avoid_: Runtime Sandbox Credential, prompt secret, Agent env secret, persisted Session secret
