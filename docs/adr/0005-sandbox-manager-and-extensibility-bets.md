...
A **Runtime CLI Credential** deliberately does not belong to the Environment Spec. At authenticated event ingress, the Host generates each Pending Event id, stores `X-VFS-Token` and the validated, allowlisted VFS project context (`X-VFS-Project-Url`, `X-VFS-Project-Id`, `X-VFS-Teamwork-Id`, `X-VFS-Storyboard-Id`, and `X-VFS-Runtime-Env`) in short-lived Redis under that id, then durably enqueues only the non-secret id and event. The Host that wins the fenced claim resolves the exact values, regardless of which Host accepted the request. The per-Turn ToolExecutor adds `VFS_TOKEN`, `VFS_PROJECT_URL`, `VFS_PROJECT_ID`, `VFS_TEAMWORK_ID`, `VFS_STORYBOARD_ID`, and `RUNTIME_ENV` only to a single direct `vfs-cli` subprocess; arbitrary shell commands and the sandbox's create-time environment never receive them. The token is never model-visible. The same non-secret project context is added to one transient system-prompt block for the exact Turn so the Agent can pass identifiers, such as storyboard id, that vfs-cli does not resolve from environment; this block is never appended to the Event Log. `sandbox.credentialMode: runtime-vfs` strips `VFS_TOKEN` from both deployment defaults and Agent env while preserving unrelated create-time values, so legacy Agents may retain the old shared-token behavior without Kiki silently falling back to it. Acknowledging the Pending Event deletes the credential and context, including the completion-marker recovery fast path. Redis loss fails closed at `vfs-cli` authentication without persisting a reusable user credential.
