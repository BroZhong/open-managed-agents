...
ADR-0001's proposed second mode; ADR-0001 is now historical, and its
Agent-in-the-Sandbox deployment recipes are no longer maintained. The presence
of Claude Code/Codex adapters does not imply a deployed sandbox template for them.
...
- **Redis** carries transient traffic only: the pending-input queue and per-turn delta streams (`stream:session:{sessionId}:turn:{turnId}`, reclaimed when the turn ends). Turn IDs are Session-local, so every delta read/write/reclaim requires the owning Session ID. Legacy global delta keys are not replayed because their ownership is unknown. Deltas are never persisted to PostgreSQL; a turn's final content is stored whole as an Event, aligned to deltas by `turnId` + `blockIndex`.
